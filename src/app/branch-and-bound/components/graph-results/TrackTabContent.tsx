          ...
          <div className="flex flex-col w-full md:w-1/4 space-y-6">
            <div>
              ...
              <p className="text-xs text-gray-600 mb-2">
                Optimal route: {result?.best?.attributes.length.toFixed(2)}
              </p>
            </div>
            
            <ScrollArea className="h-[300px]">
              <div className="flex flex-col gap-2">
                {result?.best?.attributes.path.map((source: string, index: number) => (
                  <div key={index} className="flex items-center group">
                    <div className="flex items-center bg-gray-50 hover:bg-gray-100 px-4 py-2 rounded-lg w-full transition-colors">
                      ...
                      <div className="flex flex-col">
                        ...
                        {result?.best && index < result.best.attributes.path.length - 1 ? (
                          <span className="text-xs text-gray-500">
                            To {result?.best?.attributes.path[index + 1]}: {
                              graph.links.find(
                                (link: {source : string | { id: string }, target: string | { id: string }, weight: number}) =>
                                  ...
                                  (typeof link.target === 'string' ? link.target : link.target.id) === result.best?.attributes.path[index + 1]) ||
                                  ((typeof link.target === 'string' ? link.target : link.target.id) === result.best?.attributes.path[index + 1])
                      ...
                      {index < (result?.best?.attributes.path.length ?? 0) - 1 && (
            ...
            <div className="flex flex-wrap gap-2">
              <div className="flex items-center gap-1 text-xs text-gray-600">
                ...
                <span>Total: {result?.best?.attributes.path.length ?? 0}</span>
              </div>
            </div>
          </div>
